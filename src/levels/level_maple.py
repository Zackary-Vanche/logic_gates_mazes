...
def f(fast_solution_finding=False): 
    ...
    T0 = Tree(tree_list=['AND',
                         ['EQU', ['MOD', Tree.tree_list_BIN(6), [None]], [None]],
                         ['INF', [None], [None], [None]]],
                name='T0',
                switches=[S0, S1, S2, S3, S4, S5, 9, 3,
                          0, V0, V1],
    ...
    R2 = Room(name='R2',
                ...
                switches_list=[S7])
    R3 = Room(name='R3',
                position=[0*dx-0.25, 1*dy, ex, ey],
                switches_list=[S8])
    R4 = Room(name='R4',
                position=[1*dx+0.25, 1*dy, ex, ey],
                switches_list=[S9])
    R5 = Room(name='R5',
    ...
    level = Maze(start_room_index=0,
                 ...
                 fastest_solution="S0 S3 S4 S5 D0 S10 D5 S8 D2 S6 D2 D4 S9 D3 S7 D3 D6 S11 D6 D4 D5 D8",
...
def get_color():
    lcolor = Levels_colors_list.FROM_HUE(hu=0, sa=0.4, li=0.6)
    lcolor.surrounding_color = Color.color_hls(hu=0.2, sa=0.4, li=0.6)
    lcolor.contour_color = Color.color_hls(hu=0.2, li=0.6, sa=1)
    return lcolor